...
| `written-exams.html` | FOI + FIA + FII written exam prep (FII figures load from `images/fii/`; add a course with `~/Downloads/FlightTest5-Toolkit/add_course.py`). Banks + FII figures are AES-encrypted: `node tools/exam-vault.mjs unlock` before editing data, `lock` before committing (pre-commit hook enforces; also covers cfii-stage1-drill.html) |
| `fii-study-2.html` | FII Study 2 — Prepware FII bank captured with ScreenToWord auto-walk (`~/Desktop/ScreenToWord`). Same app as written-exams (sticky search bar in Scroll, search spans all categories, own `fii2_` progress). Rebuild bank: `python3 tools/fii2_import.py` (reads `~/Documents/Screen Captures/Screen Captures.docx`; category from the "x of N" counter, legacy ids in `tools/fii2_legacy_cats.json`). Vaulted like written-exams: unlock → import → `node tools/exam-vault.mjs lock fii-study-2.html` before committing; figures in `images/fii2/`. Stage-test PDFs → `python3 tools/fii2_stage_import.py <pdfs>` (writes `~/Documents/Screen Captures/fii2_extra.json`, outside the repo; merged by the importer). FAA IR test supplement lives unencrypted in `supplement/` (public domain; `index.json` maps Figure/Legend → page); the page renders it with pdf.js (`cfi/vendor`), per-page rotation in localStorage `fii2_suprot`. One-shot publish: `tools/fii2_publish.sh` (Keychain password) |
