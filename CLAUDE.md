...
| `dme.html` + `dme.js` | DME Arc Trainer (same shell/phone layout as holds.html): Learn (what/join/10-10/leave), Explore (outbound radial → arc → inbound radial, DME, GS; lead = GS/200 NM, LR = 60×lead/DME), Practice (arc heading / join turn / lead radial; score in localStorage `dme_score`). Lead turns are sized to roll out tangent on the arc and on the final radial. |
