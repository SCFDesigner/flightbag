...
(function () {
  ...
  function e6bWind() {
    ...
    if (tc == null || tas == null || wd == null || ws == null || tas <= 0) {
      ['e6bOutWca','e6bOutHdg','e6bOutGs','e6bOutHw','e6bOutXw'].forEach(i => set(i, '—'));
      return;
    }
    ...
    if (Math.abs(sinWca) > 1) {             // wind too strong to hold course
      set('e6bOutWca', 'wind > TAS'); set('e6bOutHdg', '—'); set('e6bOutGs', '—');
    } else {
  ...
  function e6bAlt() {
    ...
    if (altim == null || elev == null) { ['e6bOutPa','e6bOutDa','e6bOutTas'].forEach(i => set(i, '—')); return; }
  ...
  const conv = {
    ...
    dist:  { toKts: { nm: v => v, sm: v => v / 1.15078 }, fromKts: { nm: v => v, sm: v => v * 1.15078 } },
  ...
  function init() {
    const panels = { wind: e6bWind, tsd: e6bTsd, fuel: e6bFuel, descent: e6bDescent, alt: e6bAlt };
